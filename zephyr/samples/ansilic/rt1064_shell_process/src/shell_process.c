...
pid_t new_task(const char *name, void *(*run)(void *), void *arg)
{
	...
	for (int i = 0; i < MAX_STACK_POOL; i++) {
		if (!stack_pool_used[i]) {
			...
			/*
			 * Keep the array symbol rather than &stack_pool[i][0].
			 * The stack region is [stack, stack + usable_size) and
			 * the thread's initial SP goes at its *top*. Taking the
			 * element address here (the array's lowest byte) made the
			 * kernel treat the bottom of the pool entry as the bottom
			 * of a region it then grew downwards out of, straight into
			 * the MPU stack guard at the head of the pool.
			 */
			stack = stack_pool[i];
			stack_index = i;
	...
	k_tid_t tid = k_thread_create(
		thread,
		/*
		 * K_THREAD_STACK_BUFFER() skips the MPU stack-guard reservation
		 * at the base of the object; passing the raw symbol would hand
		 * the kernel the guard bytes as usable stack. It returns char *,
		 * so cast back to the element type k_thread_create expects.
		 *
		 * The size is TASK_STACK_SIZE directly. The original code passed
		 * K_THREAD_STACK_SIZEOF(TASK_STACK_SIZE), i.e. the macro applied
		 * to the *number* rather than to a stack symbol: it expands to
		 * sizeof(2048) - K_THREAD_STACK_RESERVED, a negative int, which
		 * as size_t becomes ~2^64 and puts the computed stack top outside
		 * the pool entirely.
		 */
		(k_thread_stack_t *)K_THREAD_STACK_BUFFER(stack),
		TASK_STACK_SIZE,
		task_trampoline,
		tramp_arg,
		NULL,
