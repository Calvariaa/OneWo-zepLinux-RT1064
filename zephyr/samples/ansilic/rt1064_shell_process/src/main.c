...
/*
 * SD card commands. The implementations are in sd_commands.c and are reached
 * through the sample's command registry, exactly like the commands above: this
 * shell registers a native Zephyr command whose handler looks the real
 * implementation up by name and runs it in a child process.
 *
 * Guarded so a build without a filesystem sees neither the commands nor their
 * handlers.
 */
static int shell_sdinfo_handler(const struct shell *sh, size_t argc, char **argv)
{
	const struct shell_cmd *cmd = shell_cmd_lookup("sdinfo");
...
static int shell_sdtest_handler(const struct shell *sh, size_t argc, char **argv)
{
	const struct shell_cmd *cmd = shell_cmd_lookup("sdtest");
...
static int shell_sdclear_handler(const struct shell *sh, size_t argc, char **argv)
{
	const struct shell_cmd *cmd = shell_cmd_lookup("sdclear");
...
static int shell_sdwipe_handler(const struct shell *sh, size_t argc, char **argv)
{
	const struct shell_cmd *cmd = shell_cmd_lookup("sdwipe");
...
SHELL_CMD_ARG_REGISTER(sdinfo, NULL, "Show SD card capacity and free space",
                       shell_sdinfo_handler, 1, 0);
SHELL_CMD_ARG_REGISTER(sdtest, NULL, "SD write/read verification [sectors]",
                       shell_sdtest_handler, 1, 1);
SHELL_CMD_ARG_REGISTER(sdclear, NULL, "Fast clear SD (add 'format' to re-create FAT)",
                       shell_sdclear_handler, 1, 1);
SHELL_CMD_ARG_REGISTER(sdwipe, NULL, "Securely erase SD with zeros ('confirm' to run)",
                       shell_sdwipe_handler, 1, 1);
...
int main(void)
{
	printk("\n");
	printk("========================================\n");
	printk("  zepLinux Shell with Process Support\n");
	printk("  board: %s   soc: %s\n", CONFIG_BOARD_TARGET, CONFIG_SOC);
